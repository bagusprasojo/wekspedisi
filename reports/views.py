...
@login_required
def rekap_transaksi_piutang(request):
    require_tenant(request)
    filters = month_report_filters(request)
    if filters['start_date'].year != filters['end_date'].year:
        from django.contrib import messages
        messages.warning(request, 'Periode Rekap Transaksi Piutang harus berada pada tahun yang sama.')
        filters['end_date'] = date(filters['start_date'].year, 12, 31)
    rows = services.rekap_transaksi_piutang(request.tenant, filters['start_date'], filters['end_date'])
    if request.GET.get('export') in {'excel', 'pdf'}:
        period = f"{filters['start_date'].strftime('%d/%m/%Y')} s.d. {filters['end_date'].strftime('%d/%m/%Y')}"
        export_rows = [
            [
                ...
                (row.tanggal.strftime('%d/%m/%y'), 'center'),
                (row.no_register, 'text'),
                (row.penerima_pinjaman.nama if row.penerima_pinjaman else '', 'text'),
                (row.perkiraan_piutang.nama if row.perkiraan_piutang else '', 'text'),
                (row.sumber_dana, 'text'),
                (row.nominal, 'number'),
                (row.pelunasan, 'number'),
                (row.saldo, 'number'),
        ...
        headers = ['No', 'Tanggal', 'No Register', 'Penerima Pinjaman', 'Akun Piutang', 'Sumber Uang', 'Nominal', 'Pelunasan', 'Saldo', 'Pc']
        totals = [
            ('Grand Total', 'text', 6),
            (sum((row.nominal for row in rows), ZERO), 'number', 1),
            (sum((row.pelunasan for row in rows), ZERO), 'number', 1),
        ...
        if request.GET.get('export') == 'excel':
            return legacy_report_excel_response('rekap-transaksi-piutang.xls', 'Rekap Transaksi Piutang Pinjaman', request.tenant, period, headers, export_rows, totals)
        return legacy_report_pdf_response(
            'rekap-transaksi-piutang.pdf',
            'Rekap Transaksi Piutang Pinjaman',
            request.tenant,
            period,
            [
                {'label': 'No', 'x': 0, 'w': 25, 'max': 4, 'size': 8},
                {'label': 'Tanggal', 'x': 25, 'w': 60, 'max': 8, 'size': 8},
                {'label': 'No Register', 'x': 85, 'w': 120, 'max': 25, 'size': 8},
                {'label': 'Penerima Pinjaman', 'x': 205, 'w': 140, 'max': 25, 'size': 8},
                {'label': 'Akun Piutang', 'x': 345, 'w': 110, 'max': 20, 'size': 8},
                {'label': 'Sumber Uang', 'x': 455, 'w': 110, 'max': 20, 'size': 8},
                {'label': 'Nominal', 'x': 565, 'w': 75, 'max': 14, 'size': 8},
                {'label': 'Pelunasan', 'x': 640, 'w': 75, 'max': 14, 'size': 8},
                {'label': 'Saldo', 'x': 715, 'w': 75, 'max': 14, 'size': 8},
            ],
    ...
    return render(
        request,
        'reports/rekap_transaksi_piutang.html',
        {
            'title': 'Rekap Transaksi Piutang Pinjaman',
            'rows': page_obj.object_list,
            'page_obj': page_obj,
            'is_paginated': page_obj.has_other_pages(),
            'total_nominal': total_nominal,
...
@login_required
def rekap_pembayaran_piutang(request):
    require_tenant(request)
    filters = month_report_filters(request)
    if filters['start_date'].year != filters['end_date'].year:
        from django.contrib import messages
        messages.warning(request, 'Periode Rekap Pembayaran Piutang harus berada pada tahun yang sama.')
        filters['end_date'] = date(filters['start_date'].year, 12, 31)
    rows = services.rekap_pembayaran_piutang(request.tenant, filters['start_date'], filters['end_date'])
    if request.GET.get('export') in {'excel', 'pdf'}:
        period = f"{filters['start_date'].strftime('%d/%m/%Y')} s.d. {filters['end_date'].strftime('%d/%m/%Y')}"
        export_rows = [
            [
                ...
                (row.tanggal.strftime('%d/%m/%y'), 'center'),
                (row.no_register, 'text'),
                (row.piutang_pinjaman.no_register, 'text'),
                (row.piutang_pinjaman.penerima_pinjaman.nama if row.piutang_pinjaman.penerima_pinjaman else '', 'text'),
                (row.sumber_dana, 'text'),
                (row.nominal, 'number'),
                (row.created_by.username if row.created_by else '', 'text'),
            ]
        ...
        headers = ['No', 'Tanggal', 'No Register', 'No Piutang', 'Penerima Pinjaman', 'Penerima Uang', 'Pembayaran', 'Pc']
        totals = [('Grand Total', 'text', 6), (sum((row.nominal for row in rows), ZERO), 'number', 1), ('', 'text', 1)]
        if request.GET.get('export') == 'excel':
            return legacy_report_excel_response('rekap-pembayaran-piutang.xls', 'Rekap Pembayaran Piutang Pinjaman', request.tenant, period, headers, export_rows, totals)
        return legacy_report_pdf_response(
            'rekap-pembayaran-piutang.pdf',
            'Rekap Pembayaran Piutang Pinjaman',
            request.tenant,
            period,
            [
                {'label': 'No', 'x': 0, 'w': 25, 'max': 4, 'size': 8},
                {'label': 'Tanggal', 'x': 25, 'w': 60, 'max': 8, 'size': 8},
                {'label': 'No Register', 'x': 85, 'w': 125, 'max': 25, 'size': 8},
                {'label': 'No Piutang', 'x': 210, 'w': 125, 'max': 25, 'size': 8},
                {'label': 'Penerima Pinjaman', 'x': 335, 'w': 140, 'max': 25, 'size': 8},
                {'label': 'Penerima Uang', 'x': 475, 'w': 120, 'max': 20, 'size': 8},
                {'label': 'Pembayaran', 'x': 595, 'w': 85, 'max': 14, 'size': 8},
                {'label': 'Pc', 'x': 680, 'w': 50, 'max': 10, 'size': 8},
            ],
            [row[:8] for row in export_rows],
    ...
    return render(
        request,
        'reports/rekap_pembayaran_piutang.html',
        {
            'title': 'Rekap Pembayaran Piutang Pinjaman',
            'rows': page_obj.object_list,
            'page_obj': page_obj,
            'is_paginated': page_obj.has_other_pages(),
            'total_nominal': total_nominal,
...
@login_required
def saldo_piutang_pinjaman(request):
    require_tenant(request)
    filters = report_filters(request)
    end_date = filters['end_date'] or date.today()
    filters['end_date'] = end_date
    rows = services.saldo_piutang_pinjaman(request.tenant, end_date)
    total_nominal = sum((row['nominal'] for row in rows), ZERO)
    total_pelunasan = sum((row['pelunasan'] for row in rows), ZERO)
    total_saldo = sum((row['saldo'] for row in rows), ZERO)
    ...
    if request.GET.get('export') in {'excel', 'pdf'}:
        ...
        export_rows = [
            [
                ...
                (row['tanggal'].strftime('%d/%m/%y'), 'center'),
                (row['no_register'], 'text'),
                (row['penerima_pinjaman'], 'text'),
                (row['alamat_penerima_pinjaman'], 'text'),
                (row['nominal'], 'number'),
                (row['pelunasan'], 'number'),
                (row['saldo'], 'number'),
                (row['created_by'], 'text'),
        ...
        if request.GET.get('export') == 'excel':
            return legacy_report_excel_response(
                'saldo-piutang-pinjaman.xls',
                'Rekap Saldo Piutang Pinjaman',
                request.tenant,
                period,
                ['No', 'Tanggal', 'No Register', 'Penerima Pinjaman', 'Alamat', 'Nominal Pinjaman', 'Pelunasan', 'Sisa Saldo Piutang', 'Pc'],
                export_rows,
                [('Grand Total', 'text', 7), (total_saldo, 'number', 1), ('', 'text', 1)],
            )
        return legacy_report_pdf_response(
            'saldo-piutang-pinjaman.pdf',
            'Rekap Saldo Piutang Pinjaman',
            request.tenant,
            period,
            [
                ...
                {'label': 'Tanggal', 'x': 25, 'w': 60, 'max': 8, 'size': 8},
                {'label': 'No Register', 'x': 85, 'w': 130, 'max': 25, 'size': 8},
                {'label': 'Penerima Pinjaman', 'x': 215, 'w': 140, 'max': 25, 'size': 8},
                {'label': 'Alamat', 'x': 355, 'w': 130, 'max': 25, 'size': 8},
                {'label': 'Nominal', 'x': 485, 'w': 80, 'max': 14, 'size': 8},
                {'label': 'Pelunasan', 'x': 565, 'w': 80, 'max': 14, 'size': 8},
                {'label': 'Saldo', 'x': 645, 'w': 80, 'max': 14, 'size': 8},
    ...
    return render(
        request,
        'reports/saldo_piutang_pinjaman.html',
        {
            'title': 'Rekap Saldo Piutang Pinjaman',
            'rows': page_obj.object_list,
            'page_obj': page_obj,
            'is_paginated': page_obj.has_other_pages(),
            'total_nominal': total_nominal,
